...
async function connectToDb() {
    try {
        await mongoose.connect('mongodb+srv://kaviyab:Kavi224@cluster0.zv3jzli.mongodb.net/Swiggy?retryWrites=true&w=majority')
        console.log('DB connection established ;)')
        const port = process.env.port || 8000  // To have the port which is available
        //const port = 8000
...
app.post('/validate-user',async function(request, response){
    try{
        const user = await Users.findOne({  //storing in user variable 
            "email" : request.body.email,
            "password" : request.body.password
        })
        if(user){
